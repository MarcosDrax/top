...
While ReadLn(S) <> EOF do Begin
	...
	ano   := TrimStr(GetString(S,1533,4));
	docm  := GetString(S,807,16);
	imp   := 'I.S.S.Q.N.';
	contr := TrimStr(GetString(S,45,60));
	ncarn := TrimStr(GetString(S,1,8));
	e_log := TrimStr(GetString(S,225,100));
	e_com := TrimStr(GetString(S,325,50));
	e_blc := TrimStr(GetString(S,425,6));
	e_apt := TrimStr(GetString(S,431,8));
	e_bai := TrimStr(GetString(S,439,50));
	e_cid := TrimStr(GetString(S,539,50));
	e_est := TrimStr(GetString(S,589,2));
	e_cep := FormatNumeric(GetNumeric(S,591,8),'00000-000');

	i_log := TrimStr(GetString(S,599,100));
	i_com := TrimStr(GetString(S,699,50));
	i_bai := TrimStr(GetString(S,749,50));
	i_cep := FormatNumeric(GetNumeric(S,799,8),'00000-000');

	lotea := TrimStr(GetString(S,594,6));
	qdra  := TrimStr(GetString(S,664,12));
	lote  := TrimStr(GetString(S,676,12));
	dv    := TrimStr(GetString(S,8781,40));
	{---Variaveis comuns Fim---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------}  

	qtd   := 0;
	j     := 558;
	for i:= 0 to 12 do Begin
		If TrimStr(GetString(S,1527+(i*j),2)) = '' then break;
		qtd := qtd +1;
	end;


	//Localizer CPF e CNPJ (FORMATAR Campo)
	x := 0;
	y := 0;
	For i := 0 to 15 do Begin
		If SubStr(docm,i,1) <> ' ' then Begin
			x := x + 1;
			end else begin
			break;
		end;
	...
	If x < 14 then begin
		if x < 9 then begin
			...
			CPFCNPJ := FormatNumeric(GetNumeric(S,807,14),'000.000.000-00');
		end;
		end else Begin
		If y = 1 then begin
			CPFCNPJ := docm;
		end else Begin
			CPFCNPJ := FormatNumeric(GetNumeric(S,807,16),'00.000.000/0000-00');
		end;
	end;
	...
	{---Configurações Referencia do contribuinte no Registro | Barra de TD_registro 3.0 ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------}
	    ...
	    TD_idNumeroBanco    := GetString(S,1537,12);    //Referencia do Banco
	...
	{--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------}
		...
		ClearFields(CAPA,REC1);
			...
			CAPA.REC1.ENDE_CAPA[2]    := e_log+' '+e_com+' '+e_blc+' '+e_apt;
			CAPA.REC1.ENDE_CAPA[3]    := e_bai;
			CAPA.REC1.ENDE_CAPA[4]    := '';
			CAPA.REC1.ENDE_CAPA[5]    := 'CEP: '+e_cep+' - '+e_cid+' '+e_est;
...

{
		...
		ClearFields(PAGE3,REC1);
			...
			PAGE3.REC1.ANO             := ano;
			PAGE3.REC1.TOTAL           := FormatFloat(GetFloat(S,1622,12),'9.999,99');
			PAGE3.REC1.VLR_PARCEL      := FormatFloat(GetFloat(S,2180,12),'9.999,99');
			PAGE3.REC1.VLR_DESCON      := FormatFloat(GetFloat(S,1634,12),'9.999,99');
			PAGE3.REC1.TRIBUBT[1]      := TrimStr(GetString(S,1597,25));
			PAGE3.REC1.TRIB_VLR[1]     := FormatFloat(GetFloat(S,1622,12),'9.999,99');
			...
			PAGE3.REC1.ATIVIDADE       := TrimStr(GetString(S,867,30));
			PAGE3.REC1.CNPJ_CPF        := CPFCNPJ;
			PAGE3.REC1.INSC_MUNIC      := TrimStr(GetString(S,837,30));
			...
			ClearFields(PAGE4,REC1);
				...
				PAGE4.REC1.CARN_INSC[2]   := insc;
					...
					if i = 0 then begin
						...
						PAGE4.REC1.PARCELA        := TrimStr(GetString(S,1527+(i*j),2))+'/'+FormatNumeric(qtd-1,'00');
				...
				PAGE4.REC1.VENCIMENTO     := FormatDate(GetDate(S,1529+(i*j),8),'DD/MM/AAAA');
				PAGE4.REC1.TOTAL          := FormatFloat(GetFloat(S,1537+(i*j),12),'9.999,99');
				PAGE4.REC1.TRIBUBT[1]     := TrimStr(GetString(S,1597+(i*j),25));
				PAGE4.REC1.TRIB_VLR[1]    := FormatFloat(GetFloat(S,1537+(i*j),12),'9.999,99');

				COD                       := TrimStr(GetString(S,1549+(i*j),48));
