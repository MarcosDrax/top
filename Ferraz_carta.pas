...
{---Configurações--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------}
    TD_cliente                    := 'P.M. DE FERRAZ DE VASCONCELOS'; //Nome parcialmente completo
...
{---Configurações de segurança-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------}
    marcaDaguaNome                := 'P.M.F.V'; //Marca dagua prefixo do cliente 
...
PROPRIETARIO     := GetString(S,3,0);
COMPROMISSARIO   := GetString(S,0,0);
ENDER_ENTREGA    := TrimStr(GetString(S,13,0))+' '+TrimStr(GetString(S,12,0))+' '+TrimStr(GetString(S,14,0))+' '+TrimStr(GetString(S,15,0))+' '+TrimStr(GetString(S,16,0))+' '+TrimStr(GetString(S,17,0))+' '+TrimStr(GetString(S,19,0))+' '+TrimStr(GetString(S,20,0)); 
CEP_ENTREGA      :=  'CEP: ' + TrimStr(GetString(S,18,0));
END_IMOVEL       := TrimStr(GetString(S,5,0))+' '+TrimStr(GetString(S,4,0))+', '+TrimStr(GetString(S,6,0))+'  '+TrimStr(GetString(S,7,0))+'  '+TrimStr(GetString(S,10,0))+' / '+TrimStr(GetString(S,11,0));
BAIRRO_IMOVEL    := GetString(S,8,0);
CIDADE_IMOVEL    := GetString(S,10,0);
UF_IMOVEL        := GetString(S,11,0);
CEP_IMOVEL       := GetString(S,9,0);
CNPJ_CPF         := GetString(S,23,0);
QRCODE           := 'https://portal.cidadao.conam.com.br/ferrazdevasconcelos/2via_parcelamento.php';
//CODIGO           := TrimStr(GetString(S,11,0));
INSCRICAO        := TrimStr(GetString(S,22,0)); 


{---Variaveis comuns Fim---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------}  
{---Configurações Referencia do contribuinte no Registro | Barra de TD_registro 3.0 ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------}
    TD_idArquivo        := TrimStr(GetString(S,0,0));    //Identificação do arquivo maximo ate 12 numeros
    TD_idNumeroBanco    := TrimStr(GetString(S,0,0));    //Referencia do Banco
    TD_idCodigoBanco    := TrimStr(GetString(S,0,0));    //Referencia do contribuinte para com o banco (ex: nossonumero)
    //TD_idExtra          := TrimStr(GetString(S,0,0));   //Identificação extra 
{--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------}  
...
ClearFields(PAGE1,REC1); 
   PAGE1.REC1.AVISO          := GetString(S,0,0);
   PAGE1.REC1.PROPRIETAR     := PROPRIETARIO;
   PAGE1.REC1.COMPROMISS     := COMPROMISSARIO;
   PAGE1.REC1.ENDER_ENTR     := ENDER_ENTREGA; 
   PAGE1.REC1.CEP            :=  CEP_ENTREGA;
...
ClearFields(PAGE2,REC1); 
   ...
   PAGE2.REC1.NOSSO          := GetString(S,0,0);
   PAGE2.REC1.END_ENTREG     := END_IMOVEL;
   PAGE2.REC1.BAIRRO         := BAIRRO_IMOVEL;
   PAGE2.REC1.MUNICIPIO      := CIDADE_IMOVEL;
   PAGE2.REC1.UF             := UF_IMOVEL;
if getstring(s,23,0) <> 'SEM' then begin  PAGE2.REC1.CNPJ_CPF       := CNPJ_CPF;end else begin PAGE2.REC1.CNPJ_CPF       := 'Sem Documento';end;
   PAGE2.REC1.CEP            := CEP_IMOVEL;
   //PAGE2.REC1.VENCTO         := GetDate(S,0,0);
   //PAGE2.REC1.RED            := GetString(S,0,0);
   //PAGE2.REC1.BARRA          := GetString(S,0,0);
   //PAGE2.REC1.VALOR          := GetFloat(S,0,0);
   //PAGE2.REC1.EXECUCAO       := TrimStr(GetString(S,0,0));
   PAGE2.REC1.QRCODE         := QRCODE;
...
For i := 0 to 27 do Begin
	If TrimStr(GetString(S,25+(i*3),0)) <> '' then begin
		if getstring(s,22,0) = 'M' then begin
			PAGE2.REC1.INSCRICOES[1]   := 'Mobiliaria - ' +TrimStr(GetString(S,21,0));end else begin PAGE2.REC1.INSCRICOES[1]   := 'Imobiliaria - 'TrimStr(GetString(S,21,0));
		end;
		//PAGE2.REC1.PARCELAMEN[i+1]   := TrimStr(GetString(S,24,0));
		PAGE2.REC1.VENCIMENTO[i+1] := TrimStr(GetString(S,25+(i*3),0));
		PAGE2.REC1.PARCELAS_N[i+1] := TrimStr(GetString(S,24+(i*3),0));
		
		tamanho_vlr :=0;
		for j := 0 to 16 do begin
			if (SubStr(Getstring(S,26+(i*3),16),j+1,1)) <> '.' then begin
				tamanho_vlr:= tamanho_vlr + 1;
				end else begin
				break;
			end;
		end;
		tamanho_vlr := tamanho_vlr+3;

		PAGE2.REC1.VLR_ATUAL [i+1] := replace(Getstring(S,26+(i*3),tamanho_vlr),'.',',');
	end else begin
	break;
	end;
End;
