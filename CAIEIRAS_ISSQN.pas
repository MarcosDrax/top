...
While ReadLn(S) <> EOF do Begin
	...
	parcela       := 0.0;
	vlrUni1       := GetFloat(S,64,0);vlrParc:=GetFloat(S,65,0);
	AVISO_SIST    := GetString(S,54,0);
	...
	ALVARA        := GetString(S,0,0);
	LIC_FUNCIO    := GetString(S,25,0);
	ISS_ANUAL     := GetString(S,0,0);
	...
	{--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------}
		...
		//PARCELA ÚNICA
			...
			PAGE3.REC1.DATAPAC :=  GetDate(S,62,0);  
			PAGE3.REC1.NSSNUMERO :=  GetString(S,63,0);  
			PAGE3.REC1.TOTALPARC :=  GetFloat(S,64,0); 
		...
		for i := 0 to 8 do begin
			...
			PAGE4.REC1.DATAPAC :=  GetDate(S,26+(i*3),0);  
			PAGE4.REC1.NSSNUMERO :=  GetString(S,27+(i*3),0);  
			PAGE4.REC1.TOTALPARC :=  GetFloat(S,28+(i*3),0); 
