...
// | Atualização:  14/02/2023                |
// | Programa:  PREFEITURA DE GUATAPORANGA   |
// +-----------------------------------------+ 
...
{---Configurações--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------}
    TD_cliente                    := 'P.M. NOVA GUATAPORANGA'; //Nome parcialmente completo
...
NO_CAD_CON     := GetString(S,1,25);
CONTRIBUINTE   := GetString(S,51,40);
COMPROMISSARIO := GetString(S,158,50);
CODIGO_RUA     := GetString(S,104,5);
RUA_IMOVE      := GetString(S,295,50);
NUM_IMOVE      := GetString(S,240,10);
BAIRRO_IMOVE   := GetString(S,345,50);
END_ENTREG     := TrimStr(GetString(S,445,50)) + '  ' + TrimStr(GetString(S,545,10)) + '  ' + TrimStr(GetString(S,555,30))+ '  ' + TrimStr(GetString(S,495,50));
COMPL_ENTREG   := TrimStr(GetString(S,555,30));
CID_ENTREG     := GetString(S,600,40);
UF_ENTREG      := GetString(S,640,2);
CEP_ENTREG     := GetString(S,585,15);
QUADRA         := GetString(S,405,10);
LOTE           := GetString(S,415,10);
TOTAL_IMPOSTO  :=(GetFloat(S,1177,10) + GetFloat(S,1242,10) + GetFloat(S,1307,10) + GetFloat(S,1372,10))/100;
{---Variaveis comuns Fim---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------}  
{---Configurações Referencia do contribuinte no Registro | Barra de TD_registro 3.0 ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------}
    TD_idArquivo        := GetString(S,26,9);    //Identificação do arquivo maximo ate 12 numeros
...
ClearFields(PAGE1,REC1);     
     PAGE1.REC1.NO_CAD_CON := NO_CAD_CON ;
     PAGE1.REC1.NOME_CONTR := CONTRIBUINTE;
     PAGE1.REC1.NOME_COMPR := COMPROMISSARIO;
     PAGE1.REC1.COD_RUA := CODIGO_RUA;
     PAGE1.REC1.DESC_RUA := RUA_IMOVE;
     PAGE1.REC1.NUM_IMOVE := NUM_IMOVE;
     PAGE1.REC1.COMPL_ENDE := COMPL_ENTREG;
     PAGE1.REC1.DESC_BAIRR := BAIRRO_IMOVE;
     PAGE1.REC1.END_ENTREG := END_ENTREG;
     PAGE1.REC1.CID_ENTREG := CID_ENTREG;
     PAGE1.REC1.UF_ENTREG := UF_ENTREG;
...
//page 03 demonsotrativo de calculos
BeginPage(PAGE3);
ClearFields(PAGE3,REC1);
     PAGE3.REC1.NO_CAD_CON := NO_CAD_CON ;
     PAGE3.REC1.NOME_CONTR := CONTRIBUINTE;
     PAGE3.REC1.DESC_RUA :=  RUA_IMOVE;
     PAGE3.REC1.NUM_IMOVE := NUM_IMOVE;
     PAGE3.REC1.COMPL_ENDE := COMPL_ENTREG;
     PAGE3.REC1.DESC_BAIRR := BAIRRO_IMOVE;
     PAGE3.REC1.QUADRA := QUADRA;
     PAGE3.REC1.LOTE := LOTE;
     PAGE3.REC1.END_ENTREG := END_ENTREG;
     PAGE3.REC1.CID_ENTREG := CID_ENTREG;
     PAGE3.REC1.CEP_ENTREG := CEP_ENTREG;
     PAGE3.REC1.UF_ENTREG := UF_ENTREG;
     PAGE3.REC1.NOME_COMPR := COMPROMISSARIO;
     PAGE3.REC1.AREA_TERR := GetFloat(S,727,10)/100;
     PAGE3.REC1.EDIF_IMOVE := GetFloat(S,717,10)/100;
     PAGE3.REC1.VL_VE_TERR := GetFloat(S,677,10)/100;
     PAGE3.REC1.VL_VN_EDIF := GetFloat(S,667,10)/100;
     PAGE3.REC1.VL_VN_IMOV := GetFloat(S,697,10)/100;
     PAGE3.REC1.VL_IMP_PRE := GetFloat(S,1177,10)/100;
     PAGE3.REC1.TEST_PRINC := GetFloat(S,827,10)/100;
     PAGE3.REC1.QTD_PARCEL := GetString(S,917,3);
     PAGE3.REC1.CONSERV := GetFloat(S,1372,10)/100;
     PAGE3.REC1.EXPEDIENTE := GetFloat(S,1242,10)/100;
     PAGE3.REC1.LIMPEZA := GetFloat(S,569,10)/100;
     PAGE3.REC1.REMOCAO := GetFloat(S,1307,10)/100;
     PAGE3.REC1.N_CLASS := GetFloat(S,629,10)/100;
     PAGE3.REC1.VL_TOTAL := TOTAL_IMPOSTO;
    PAGE3.REC1.AVS:=AVISO;
    PAGE3.REC1.N_ARQUIV:=TD_nomeDoArquivo;
	PAGE3.REC1.BARRA := barraDeControle;    
...
ClearFields(PAGE4,REC1);
     PAGE4.REC1.NO_CAD_CON :=NO_CAD_CON ;
     PAGE4.REC1.NOME_CONTR := CONTRIBUINTE;
     PAGE4.REC1.QUADRA := QUADRA;
     PAGE4.REC1.LOTE := LOTE;
     PAGE4.REC1.VENC_UNICA := GetString(S,2122,10);
     PAGE4.REC1.VL_UNICA := GetFloat(S,2340,10)/100;
     PAGE4.REC1.COD_BARRA := GetString(S,2155,44);
     PAGE4.REC1.CAMP_LIVRE := GetString(S,2199,60);
...
//page 05 parcela 01 até 04
for i := 0 to 3 do begin
BeginPage(PAGE5);
ClearFields(PAGE5,REC1);     
     PAGE5.REC1.PARCELAS := GetString(S,4736+(i*868),3); 
     PAGE5.REC1.NO_CAD_CON := NO_CAD_CON ;
     PAGE5.REC1.NOME_CONTR := CONTRIBUINTE;
     PAGE5.REC1.QUADRA := QUADRA;
     PAGE5.REC1.LOTE := LOTE;
     PAGE5.REC1.VENC_PARC := GetString(S,4726+(i*868),10);
     PAGE5.REC1.VL_PARCELA := GetFloat(S,4944+(i*868),10)/100;
     PAGE5.REC1.COD_BARRA := GetString(S,4759+(i*868),44);
     PAGE5.REC1.CAMP_LIVRE := GetString(S,4803+(i*868),60);
...
//page 06 demonsotrativo de calculos
BeginPage(PAGE6);
ClearFields(PAGE6,REC1);
     PAGE6.REC1.NO_CAD_CON := NO_CAD_CON;
     PAGE6.REC1.NOME_CONTR := CONTRIBUINTE;
     PAGE6.REC1.DESC_RUA := RUA_IMOVE;
     PAGE6.REC1.NUM_IMOVE := NUM_IMOVE;
     PAGE6.REC1.COMPL_ENDE := COMPL_ENTREG;
     PAGE6.REC1.DESC_BAIRR := BAIRRO_IMOVE;
     PAGE6.REC1.QUADRA := QUADRA;
     PAGE6.REC1.LOTE := LOTE;
     PAGE6.REC1.END_ENTREG := END_ENTREG;
     PAGE6.REC1.CID_ENTREG := CID_ENTREG;
     PAGE6.REC1.CEP_ENTREG := CEP_ENTREG;
     PAGE6.REC1.UF_ENTREG := UF_ENTREG;
     PAGE6.REC1.NOME_COMPR := COMPROMISSARIO;
     PAGE6.REC1.AREA_TERR := GetFloat(S,727,10)/100;
     PAGE6.REC1.EDIF_IMOVE := GetFloat(S,717,10)/100;
     PAGE6.REC1.VL_VE_TERR := GetFloat(S,677,10)/100;
     PAGE6.REC1.VL_VN_EDIF := GetFloat(S,667,10)/100;
     PAGE6.REC1.VL_VN_IMOV := GetFloat(S,697,10)/100;
     PAGE6.REC1.VL_IMP_PRE := GetFloat(S,1177,10)/100;
     PAGE6.REC1.TEST_PRINC := GetFloat(S,827,10)/100;
     PAGE6.REC1.QTD_PARCEL := GetString(S,917,3);
     PAGE6.REC1.CONSERV := GetFloat(S,1372,10)/100;
     PAGE6.REC1.EXPEDIENTE := GetFloat(S,1242,10)/100;
     PAGE6.REC1.LIMPEZA := GetFloat(S,569,10)/100;
     PAGE6.REC1.REMOCAO := GetFloat(S,1307,10)/100;
     PAGE6.REC1.N_CLASS := GetFloat(S,629,10)/100;
     PAGE6.REC1.VL_TOTAL := TOTAL_IMPOSTO;  
